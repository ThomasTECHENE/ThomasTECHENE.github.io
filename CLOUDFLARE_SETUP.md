...
The Worker allows browser requests from `https://politcheatsheet.github.io` and `https://politcheatsheet.mytho123.ovh`. Keep `ALLOWED_ORIGINS` in `backend/wrangler.jsonc` synchronized with the published site domains. It provides public read routes and password-protected write routes.
